...
export type ExpensesState = Readonly<typeof initialState>;

// Reducer

export default (state: ExpensesState = initialState, action): ExpensesState => {
  switch (action.type) {
...
export const getEntities: ICrudGetAllAction<IExpenses> = (page, size, sort) => ({
  type: ACTION_TYPES.FETCH_EXPENSES_LIST,
  payload: axios.get<IExpenses>(`${apiUrl}?cacheBuster=${new Date().getTime()}`)
});

export const getEntity: ICrudGetAction<IExpenses> = id => {
  const requestUrl = `${apiUrl}/${id}`;
  return {
    type: ACTION_TYPES.FETCH_EXPENSES,
    payload: axios.get<IExpenses>(requestUrl)
  };
};

export const createEntity: ICrudPutAction<IExpenses> = entity => async dispatch => {
...
export const updateEntity: ICrudPutAction<IExpenses> = entity => async dispatch => {
  const result = await dispatch({
...
export const deleteEntity: ICrudDeleteAction<IExpenses> = id => async dispatch => {
  const requestUrl = `${apiUrl}/${id}`;
  const result = await dispatch({
